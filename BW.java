...
public class BW extends Classifier implements Serializable, OptionHandler {
    ...
    public void train(DataSet dataset) throws Exception{
        ...
        for(Example ex : ds.getExamples()){
            ...
            double y_sign = sign(this.u, this.v, ex);
    ...
    public void updateu(Example example, double label){
        for(int i=0; i<this.u.size(); i++){
            if(example.get(i) > 0){
                if(label > 0){
                    this.u.set(i, this.u.get(i) * alpha);
                }
                else{
                    this.u.set(i, this.u.get(i) * beta);
                }
            }
        }
    }
    ...
    public void updatev(Example example, double label){
        for(int i=0; i<this.v.size(); i++){
            if(example.get(i) > 0){
                if(label > 0){
                    this.v.set(i, this.v.get(i) * beta);
                }
                else{
                    this.v.set(i, this.v.get(i) * alpha);
                }
            }
        }
    }
